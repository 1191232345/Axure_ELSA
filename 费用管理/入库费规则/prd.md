...
现状：入库费规则配置分散，缺乏统一的分类管理，无法清晰展示二级收费项，操作记录追溯困难。

痛点：
- 入库费规则按分类展示不清晰，用户需要在多个页面切换
- 二级收费项（如SKU超量费、超重费等）无法在主表中直观展示
...
2、优化主表结构，添加二级分类列，清晰展示收费项的细分类型；添加操作人和操作时间列，完整记录规则的创建和更新信息。
...
**流程说明**：
- 新增规则时，收费分类默认为当前选中的Tab分类
- 收费项选择"卸货费"时，展示费率标准表格，支持添加多个规格价格
- 收费项选择"入库附加费"时，需要选择二级收费项，并配置阶梯价格
- 保存时自动记录创建人和创建时间
...
| 分类展示 | 收费项列表展示 | 展示当前分类下的所有收费项 | P0 |
| 规则管理 | 新增规则 | 新增入库费规则，支持费率标准和阶梯价格配置 | P0 |
| 规则管理 | 编辑规则 | 编辑已有入库费规则 | P0 |
| 规则管理 | 删除规则 | 删除入库费规则 | P0 |
| 规则管理 | 查看详情 | 查看收费项详细信息 | P1 |
| 数据导出 | 导出CSV | 导出当前分类下的收费项数据 | P1 |
...
|-----------|----------|----------|----------|----------|
| Tab切换 | 点击分类Tab | 无 | 1.更新当前分类<br>2.筛选该分类下的收费项<br>3.刷新列表 | 显示对应分类的收费项 |
| 列表加载 | 页面初始化/Tab切换 | 无 | 1.获取数据<br>2.渲染表格<br>3.更新统计信息 | 显示收费项列表 |
...
| 收费项列表加载 | 根据选中的分类Tab展示对应收费项 | 入库费规则配置表 | 按创建时间倒序排列 |
| 统计信息展示 | 显示当前分类下的收费项总数 | 前端计算 | 格式：[分类名称] - 共 X 个收费项 |
...
| 导出 | 页面右上角 | 导出当前分类下的收费项数据为CSV文件 | 当前分类下有数据 | 下载CSV文件 |
| 查看详情 | 每行操作列 | 打开收费项详情弹窗 | 无 | 展示详细信息，只读模式 |
...
|------|----------|----------|----------|
| 序号 | 前端计算 | 从1开始递增 | 数字 |
| 收费项 | 键盘输入 | 从下拉列表选择：卸货费、入库附加费 | 文本显示 |
| 二级分类 | 手工选择 | 当收费项为"入库附加费"时显示，可选：SKU超量费、超重费、清单费等 | 文本显示 |
...
| 字段 | 输入方式 | 必填 | 取值规则 |
|------|----------|------|----------|
| 收费分类 | 禁用 | - | 显示当前选中的Tab分类，不可修改 |
| 收费项 | 下拉选择 | 是 | 从下拉列表选择：卸货费、入库附加费 |
| 二级收费项 | 下拉选择 | 条件必填 | 当收费项为"入库附加费"时必填，可选：SKU超量费、超重费、清单费、轻点费、分货费、拆拖、分货、超重、清点、SKU超重费 |
| 计费单位 | 下拉选择 | 是 | 从下拉列表选择：柜、票、件、箱、托、KG、个 |
| 费率标准 | 表格录入 | 条件必填 | 当收费项为"卸货费"时显示，支持添加多行：规格、价格、备注 |
| 阶梯价格 | 表格录入 | 条件必填 | 当选择二级收费项时显示，支持添加多行：开始量、结束量、价格 |
| 条件说明 | 文本输入 | 否 | 描述收费规则的适用条件 |
| 计费举例 | 文本输入 | 否 | 提供计费示例说明 |
| 备注 | 文本输入 | 否 | 补充说明信息 |
...
| 触发条件 | 交互逻辑 | 展示内容 |
|----------|----------|----------|
| 打开新增弹窗 | 1. 收费分类默认为当前选中的Tab分类<br>2. 根据分类加载对应的收费项选项<br>3. 默认显示第一个收费项 | 显示新增弹窗，收费分类字段禁用 |
| 选择收费项="卸货费" | 1. 显示"费率标准"表格区域<br>2. 隐藏"二级收费项"选择框<br>3. 隐藏"阶梯价格"表格<br>4. 显示"费率规则"、"重量阶梯"等文本输入区 | 费率标准表格（规格、价格、备注） |
| 选择收费项="入库附加费" | 1. 显示"二级收费项"选择框<br>2. 隐藏"费率标准"表格<br>3. 显示"费率规则"、"重量阶梯"等文本输入区 | 二级收费项下拉选择框 |
| 选择二级收费项 | 1. 显示"阶梯价格"表格<br>2. 隐藏"费率规则"、"重量阶梯"等文本输入区 | 阶梯价格表格（开始量、结束量、价格） |
...
|------|------|----------|----------|----------|----------|--------------|
| 收费分类 | 收费项所属分类 | 系统生成 | 默认为当前选中的Tab分类，禁用不可修改 | 文本显示 | 64 | 字符型 |
| 收费项 | 收费项目名称 | 手工选择 | 从下拉列表选择：卸货费、入库附加费；选择后触发不同的表单展示逻辑 | 下拉选择 | 64 | 字符型 |
| 二级收费项 | 收费项的二级分类 | 手工选择 | 当收费项为"入库附加费"时显示并必填；可选：SKU超量费、超重费、清单费、轻点费、分货费、拆拖、分货、超重、清点、SKU超重费 | 下拉选择 | 64 | 字符型 |
| 计费单位 | 收费的计价单位 | 手工选择 | 从下拉列表选择：柜、票、件、箱、托、KG、个 | 下拉选择 | 32 | 字符型 |
| 费率标准 | 不同规格的价格标准 | 键盘输入 | 当收费项为"卸货费"时显示表格，支持添加多行：规格（如20GP）、价格（数字）、备注（文本） | 表格录入 | - | JSON |
| 阶梯价格 | 按数量范围的价格阶梯 | 键盘输入 | 当选择二级收费项时显示表格，支持添加多行：开始量（数字）、结束量（数字）、价格（数字） | 表格录入 | - | JSON |
| 费率规则 | 费率规则文本描述 | 键盘输入 | 当收费项为"入库附加费"但未选择二级收费项时显示，每行一条：规格\|价格\|备注 | 多行文本 | 512 | 字符型 |
| 重量阶梯 | 重量阶梯价格描述 | 键盘输入 | 可选，每行一条：范围\|价格\|单位 | 多行文本 | 512 | 字符型 |
| 条件说明 | 收费规则的适用条件 | 键盘输入 | 可选，描述收费规则的适用条件，如"单箱重量超过35kg时收取超重费" | 多行文本 | 256 | 字符型 |
| 计费举例 | 计费示例说明 | 键盘输入 | 可选，提供计费示例说明，帮助用户理解计费规则 | 多行文本 | 512 | 字符型 |
