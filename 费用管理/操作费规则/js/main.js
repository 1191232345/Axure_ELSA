...
async function loadData() {
  try {
    const resp = await fetch('data/operating-fee-rule-data.json');
    if (resp.ok) {
...
function getDefaultData() {
  return [
    {
      id: 1, ruleName: '操作费规则-标准', warehouseCode: 'CB001', warehouseName: 'CB001',
      customerCode: 'DEMO - demo', customerName: '某电子商务有限公司',
      logisticsProduct: 'NBFPS', logisticsProductCount: 19,
      operatingFeeLevel: '自定义', validityPeriod: '2019-05-01 00:00:00-2025-02-14 23:59:59',
      publishStatus: 'published', deliveryChannel: 'SPOIRET', deliveryChannelCount: 1,
      remark: '-', updater: 'zsw', creator: 'zsw',
      updateTime: '2025-04-22 17:15:28', createTime: '2025-04-22 17:14:31'
    },
    {
      id: 2, ruleName: 'VIP操作费', warehouseCode: 'WH002', warehouseName: 'WH002',
      customerCode: 'VIP001', customerName: 'VIP客户公司',
      logisticsProduct: 'FEDEX_GROUND', logisticsProductCount: 5,
      operatingFeeLevel: 'VIP', validityPeriod: '2024-01-01 00:00:00-2025-12-31 23:59:59',
      publishStatus: 'published', deliveryChannel: 'FEDEX', deliveryChannelCount: 3,
      remark: 'VIP专属费率', updater: 'admin', creator: 'admin',
      updateTime: '2025-04-20 09:30:00', createTime: '2025-04-20 09:00:00'
    },
    {
      id: 3, ruleName: '跨境操作费', warehouseCode: 'CB002', warehouseName: 'CB002',
      customerCode: 'DEMO - demo', customerName: '某电子商务有限公司',
      logisticsProduct: 'USPS_PRIORITY', logisticsProductCount: 12,
      operatingFeeLevel: '标准', validityPeriod: '2025-01-01 00:00:00-2025-06-30 23:59:59',
      publishStatus: 'draft', deliveryChannel: 'USPS', deliveryChannelCount: 2,
      remark: '-', updater: 'zsw', creator: 'zsw',
      updateTime: '2025-04-22 10:00:00', createTime: '2025-04-22 10:00:00'
    },
    {
      id: 4, ruleName: '冷链操作费', warehouseCode: 'CL001', warehouseName: 'CL001',
      customerCode: 'COLD001', customerName: '冷链物流公司',
      logisticsProduct: 'UPS_GROUND', logisticsProductCount: 8,
      operatingFeeLevel: '自定义', validityPeriod: '2024-06-01 00:00:00-2025-06-01 23:59:59',
      publishStatus: 'published', deliveryChannel: 'UPS', deliveryChannelCount: 4,
      remark: '冷链特殊操作费', updater: 'admin', creator: 'zsw',
      updateTime: '2025-04-18 14:20:00', createTime: '2025-04-15 11:00:00'
    },
    {
      id: 5, ruleName: '标准操作费-华东', warehouseCode: 'EC001', warehouseName: 'EC001',
      customerCode: 'DEMO - demo', customerName: '某电子商务有限公司',
      logisticsProduct: 'DHL_EXPRESS', logisticsProductCount: 25,
      operatingFeeLevel: '标准', validityPeriod: '2025-03-01 00:00:00-2026-03-01 23:59:59',
      publishStatus: 'cancelled', deliveryChannel: 'DHL', deliveryChannelCount: 6,
      remark: '-', updater: 'zsw', creator: 'admin',
      updateTime: '2025-04-22 17:15:28', createTime: '2025-04-10 08:00:00'
    }
  ];
}
...
function filterData() {
  ...
  const deliveryChannel = document.getElementById('filterDeliveryChannel').value;
  const operatingFeeLevel = document.getElementById('filterOperatingFeeLevel').value;
  ...
  filteredData = allData.filter(item => {
    ...
    if (deliveryChannel && item.deliveryChannel !== deliveryChannel) return false;
    if (operatingFeeLevel && item.operatingFeeLevel !== operatingFeeLevel) return false;
    if (publishStatus && item.publishStatus !== publishStatus) return false;
    if (createTimeStart && item.createTime < createTimeStart + ' 00:00:00') return false;
    if (createTimeEnd && item.createTime > createTimeEnd + ' 23:59:59') return false;
    return true;
  });
...
function resetFilters() {
  ...
  document.getElementById('filterDeliveryChannel').value = '';
  document.getElementById('filterOperatingFeeLevel').value = '';
  ...
  tbody.innerHTML = pageData.map(item => {
    ...
    return `
      <tr>
        ...
        <td>
          <span class="product-tag">${item.logisticsProduct}<span class="count">数量：${item.logisticsProductCount}</span></span>
        </td>
        <td>${item.operatingFeeLevel}</td>
...
function viewRow(id) {
  ...
  showModal('查看操作费规则 - ' + item.ruleName, `
    <div class="detail-grid">
      ...
      <div class="detail-item"><span class="detail-label">物流产品</span><span class="detail-value">${item.logisticsProduct} (数量：${item.logisticsProductCount})</span></div>
      <div class="detail-item"><span class="detail-label">操作费等级</span><span class="detail-value">${item.operatingFeeLevel}</span></div>
      <div class="detail-item"><span class="detail-label">配送渠道</span><span class="detail-value">${item.deliveryChannel} (数量：${item.deliveryChannelCount})</span></div>
...
function createRule() {
  showModal('创建操作费规则', `
    <div class="detail-grid">
      ...
      <div class="detail-item"><span class="detail-label">物流产品 <em>*</em></span><span class="detail-value"><input type="text" id="createLogisticsProduct" class="modal-input" placeholder="请输入物流产品"></span></div>
      <div class="detail-item"><span class="detail-label">操作费等级 <em>*</em></span><span class="detail-value">
        <select id="createOperatingFeeLevel" class="modal-input"><option value="">请选择</option><option value="自定义">自定义</option><option value="标准">标准</option><option value="VIP">VIP</option></select>
      </span></div>
      <div class="detail-item"><span class="detail-label">配送渠道</span><span class="detail-value"><input type="text" id="createDeliveryChannel" class="modal-input" placeholder="请输入配送渠道"></span></div>
  ...
  `, function() {
    ...
    const logisticsProduct = document.getElementById('createLogisticsProduct').value.trim();
    const operatingFeeLevel = document.getElementById('createOperatingFeeLevel').value;
    if (!ruleName || !warehouseCode || !logisticsProduct || !operatingFeeLevel) {
      showToast('请填写必填项（规则名称、仓库代码、物流产品、操作费等级）', 'warning');
      return false;
    }
...
// 导出数据
function exportData() {
  const headers = ['规则名称', '仓库', '客户代码', '物流产品', '操作费等级', '有效期', '发布状态', '配送渠道', '备注', '更新人', '创建人', '修改时间', '创建时间'];
  const rows = filteredData.map(item => [
    item.ruleName, item.warehouseCode, item.customerCode, item.logisticsProduct,
    item.operatingFeeLevel, item.validityPeriod,
    STATUS_MAP[item.publishStatus]?.label || item.publishStatus,
    item.deliveryChannel, item.remark, item.updater, item.creator, item.updateTime, item.createTime
  ]);
  ...
  link.download = '操作费规则_' + new Date().toISOString().slice(0,10) + '.csv';
  link.click();
  showToast('导出成功');
}
...
// ============ 配置加载 ============
var STORAGE_KEY = 'pageConfig_操作费规则';
var pageConfig = null;
