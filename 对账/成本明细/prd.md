...
**弹窗结构**：
弹窗分为左右两个区域：
- **左侧区域**：检索条件区域，包含基础信息（账单月、航期、账单类型）和检索条件（客户代码、仓库、时间条件、其他条件）
- **右侧区域**：费用列表区域，展示查询到的费用数据
...
| 交互场景 | 触发条件 | 执行动作 | 说明 |
|----------|----------|----------|------|
| 打开弹窗 | 点击"费用获取"按钮 | 1.初始化客户代码为"请选择"<br>2.初始化仓库为"全部"<br>3.更新检索条件显示 | 弹窗打开时的初始化操作 |
| 账单类型变更 | 选择账单类型 | 1.更新表格表头<br>2.更新检索条件显示<br>3.清空费用列表检索条件<br>4.清空数据列表<br>5.自动触发获取数据 | 类型变更时自动刷新数据 |
| 客户代码变更 | 选择客户代码 | 1.清空费用列表检索条件<br>2.清空数据列表<br>3.自动触发获取数据 | 客户代码变更时自动刷新数据 |
| 获取数据 | 点击"获取数据"按钮 | 1.收集检索条件<br>2.调用查询接口<br>3.渲染费用列表 | 手动触发数据查询 |
| 重置 | 点击"重置"按钮 | 1.清空所有检索条件<br>2.清空数据列表<br>3.更新检索条件显示 | 重置为初始状态 |

**费用列表检索条件联动**：
- 左侧检索条件区域的客户代码、仓库变更时，会同步清空右侧费用列表区域的检索条件
- 费用列表区域支持独立的检索条件（如出库单号、客户代码、仓库等），用于二次筛选
...
**弹窗结构**：
弹窗分为左右两个区域：
- **左侧区域**：检索条件区域，包含基础信息（账单月、航期、费用类型）和检索条件（客户代码、仓库、时间条件、其他条件）
- **右侧区域**：费用列表区域，展示查询到的费用数据，支持多选
...
| 交互场景 | 触发条件 | 执行动作 | 说明 |
|----------|----------|----------|------|
| 打开弹窗 | 点击"批量获取费用"按钮 | 1.检查是否有任务正在执行<br>2.初始化客户代码为"全部"<br>3.初始化仓库为"全部"<br>4.更新检索条件显示 | 弹窗打开时的初始化操作 |
| 费用类型变更 | 选择费用类型 | 1.更新检索条件显示<br>2.清空数据列表<br>3.清空选中项 | 类型变更时重置数据 |
| 获取费用 | 点击"获取费用"按钮 | 1.收集检索条件<br>2.调用查询接口<br>3.渲染费用列表 | 触发数据查询 |
| 全选/取消全选 | 点击表头复选框 | 1.切换所有行的选中状态<br>2.更新选中数量和金额合计 | 批量选择操作 |
| 单选 | 点击行复选框 | 1.切换当前行选中状态<br>2.更新选中数量和金额合计 | 单条选择操作 |
| 确认添加 | 点击"确认添加"按钮 | 1.获取选中的费用项（未选则取全部）<br>2.添加到主列表<br>3.关闭弹窗<br>4.刷新主列表 | 确认并添加数据 |
| 关闭 | 点击"关闭"或X按钮 | 1.关闭弹窗 | 关闭弹窗 |

**异步处理机制**：
- 批量获取费用采用异步处理机制，点击"获取费用"后关闭弹窗
- 系统在后台异步处理数据查询任务
- 处理完成后通过通知提示用户
- 用户可以重新打开弹窗查看结果
