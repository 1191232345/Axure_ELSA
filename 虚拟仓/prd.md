...
海外仓业务中，客户库存统一归属在公司层面，无法按部门细分管理。各部门无法独立运营、查询库存、调配发货，导致库存管理混乱、账单无法分部门结算。

### Proposed Solution 解决方案

构建虚拟仓管理系统，支持客户自建组织架构（公司→部门），为每个部门关联独立虚拟仓库。虚拟仓通过库位实现逻辑隔离，支持部门内/部门间库存调拨，按部门生成发货账单。
...
创建部门 ➡ 创建虚拟仓 ➡ 库存调拨 ➡ 发货出库 ➡ 账单结算

### 2.3 User Flows 用户流程

**调拨流程：**
- **部门内调拨**：发起 → 锁定库存 → 完成（立即生效）
- **部门间调拨**：发起 → 锁定库存 → 等待确认 → 确认 → 库存转移 → 完成
